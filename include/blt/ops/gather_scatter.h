...
// HOST pointers (callers compute them from bytes and patch tables on the
// CPU); the CUDA implementations mirror them into temporary device buffers
// internally, so model code never branches on the backend.
//
// Dispatch is keyed on the first tensor argument's backend.

// out[i, :] = table[ids_host[i], :]; ids_host holds seq_len byte values,
// each validated against the table's row count.
...
// io[i, :] += scale-independent table row: io[i, :] += table[idx_host[i], :]
// for every i whose index is not BLT_IDX_SENTINEL. Used for n-gram adds.
