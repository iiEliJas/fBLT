...
void blt_softmax_backward_cpu(const blt_tensor *grad_out, const blt_tensor *softmax_out, blt_tensor *grad_in) {
    ...
    BLT_REQUIRE(grad_out->dtype == BLT_DTYPE_FP32 && softmax_out->dtype == BLT_DTYPE_FP32 &&
                    grad_in->dtype == BLT_DTYPE_FP32,
                "blt_softmax_backward: all tensors must be FP32");
...
void blt_cross_entropy_forward_cpu(const blt_tensor *logits, const blt_tensor *targets, blt_tensor *loss_out) {
    ...
    blt_check_nd_fp32(logits, 2, (const size_t[]){0, 0},
                      "blt_cross_entropy_forward: logits must be 2D FP32 [seq_len, vocab_size]");
    ...
    const uint8_t *td = (const uint8_t *)targets->data;
    ...
    for (size_t i = 0; i < seq_len; i++) {
        ...
        uint8_t target = td[i];
        BLT_REQUIRE((size_t)target < vocab_size, "blt_cross_entropy_forward: target index out of range for vocab_size");
...
void blt_cross_entropy_backward_cpu(const blt_tensor *logits, const blt_tensor *targets, blt_tensor *grad_logits) {
    ...
    blt_check_nd_fp32(logits, 2, (const size_t[]){0, 0},
                      "blt_cross_entropy_backward: logits must be 2D FP32 [seq_len, vocab_size]");
...
// RoPE backward: the jacobian is an orthogonal 2x2 rotation matrix, so the
// backward pass is just the inverse rotation (flip the sign of sin).
...
void blt_rmsnorm_backward_cpu(const blt_tensor *grad_out, const blt_tensor *x, const blt_tensor *weight,
    ...
    blt_check_nd_fp32(grad_out, 2, (const size_t[]){seq_len, embed_dim},
                      "RMSNorm backward: grad_out shape/dtype mismatch");
    blt_check_nd_fp32(grad_x, 2, (const size_t[]){seq_len, embed_dim}, "RMSNorm backward: grad_x shape/dtype mismatch");
    blt_check_nd_fp32(grad_weight, 1, (const size_t[]){embed_dim},
                      "RMSNorm backward: grad_weight must be [embed_dim] FP32");
