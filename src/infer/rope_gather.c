...
void blt_rope_position_gather(
    ...
    BLT_REQUIRE(cos_cache != NULL && sin_cache != NULL && positions != NULL &&
                cos_out != NULL && sin_out != NULL && arena != NULL,
    ...
    for (size_t i = 0; i < n; i++) {
        BLT_REQUIRE(positions[i] < max_seq_len,
            "blt_rope_position_gather: positions[%zu]=%zu out of cache range [0, %zu)",
