...
blt_kv_cache* blt_kv_cache_create(blt_arena* arena, const blt_local_decoder* decoder,
    ...
    BLT_REQUIRE(c->cached_patches != NULL,
        "blt_kv_cache_create: failed to allocate patch table");
...
void blt_kv_decode_step(blt_kv_cache* cache,
    ...
    BLT_REQUIRE(q_group_ids != NULL && kv_group_ids != NULL,
        "blt_kv_decode_step: failed to allocate group id tables");
    ...
    for (size_t l = 0; l < L; l++) {
        ...
        for (size_t h = 0; h < H; h++) {
            ...
            blt_strided_copy(backend, vd + base * E + q_off, E,
                             (const float*)qkv.data + v_off, stride3, n, hd);
        }
