...
static int blt_cuda_numel(const blt_tensor* t) {
    return (int)t->numel;
}
...
extern "C" void blt_cast_cuda(const blt_tensor* in, blt_tensor* out) {
    ...
    if (in->dtype == BLT_DTYPE_FP32 && out->dtype == BLT_DTYPE_BF16) {
        cast_fp32_to_bf16_kernel<<<blocks, threads>>>(
            (const float*)in->data, (__nv_bfloat16*)out->data, n);
        return;
    }

    if (in->dtype == BLT_DTYPE_BF16 && out->dtype == BLT_DTYPE_FP32) {
        cast_bf16_to_fp32_kernel<<<blocks, threads>>>(
            (const __nv_bfloat16*)in->data, (float*)out->data, n);
        return;
    }
