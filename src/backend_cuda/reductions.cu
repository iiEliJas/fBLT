...
extern "C" void blt_softmax_cuda(const blt_tensor *in, blt_tensor *out) {
    ...
    blt_softmax_kernel<<<blt_cuda_grid(rows), 256>>>((const float *)in->data, (float *)out->data, rows, last_dim);
    blt_cuda_launch_check("blt_softmax");
}

// dx_i = y_i * (dy_i - sum_j(dy_j * y_j)), applied per row over the last dim.
__global__ void blt_softmax_backward_kernel(const float *dy, const float *y, float *dx, size_t rows, size_t last_dim) {
    for (size_t row = (size_t)blockIdx.x * blockDim.x + threadIdx.x; row < rows;
         row += (size_t)gridDim.x * blockDim.x) {
        const size_t base = row * last_dim;
        float dot = 0.0f;
...
extern "C" void blt_softmax_backward_cuda(const blt_tensor *grad_out, const blt_tensor *softmax_out,
    ...
    blt_softmax_backward_kernel<<<blt_cuda_grid(rows), 256>>>(
        (const float *)grad_out->data, (const float *)softmax_out->data, (float *)grad_in->data, rows, last_dim);
...
__global__ void blt_ce_row_loss_kernel(const float *logits, const unsigned char *targets, float *row_loss,
                                       size_t seq_len, size_t vocab_size) {
    for (size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x; i < seq_len; i += (size_t)gridDim.x * blockDim.x) {
...
extern "C" void blt_cross_entropy_forward_cuda(const blt_tensor *logits, const blt_tensor *targets,
    ...
    float *row_loss = (float *)blt_arena_alloc(blt_cuda_get_scratch_arena(), seq_len * sizeof(float), 16);
    blt_ce_row_loss_kernel<<<blt_cuda_grid(seq_len), 256>>>(
        (const float *)logits->data, (const unsigned char *)targets->data, row_loss, seq_len, vocab_size);
    cudaError_t err = cudaGetLastError();
    if (err == cudaSuccess) {
...
__global__ void blt_cross_entropy_backward_kernel(const float *logits, const unsigned char *targets, float *grad,
                                                  size_t seq_len, size_t vocab_size, float inv_seq_len) {
    for (size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x; i < seq_len; i += (size_t)gridDim.x * blockDim.x) {
        const float *row = logits + i * vocab_size;
...
extern "C" void blt_cross_entropy_backward_cuda(const blt_tensor *logits, const blt_tensor *targets,
    ...
    blt_cross_entropy_backward_kernel<<<blt_cuda_grid(seq_len), 256>>>(
        (const float *)logits->data, (const unsigned char *)targets->data, (float *)grad_logits->data, seq_len,
        vocab_size, 1.0f / (float)seq_len);
    blt_cuda_launch_check("blt_cross_entropy_backward");
}

// RoPE

__global__ void blt_rope_precompute_kernel(float *cos_out, float *sin_out, size_t max_seq_len, size_t half, float theta,
                                           size_t head_dim) {
...
extern "C" void blt_rope_precompute_cuda(size_t max_seq_len, const blt_rope_config *config, blt_tensor *cos_out,
                                         blt_tensor *sin_out) {
    const size_t half = config->head_dim / 2;
    const size_t count = max_seq_len * half;
    blt_rope_precompute_kernel<<<blt_cuda_grid(count), 256>>>((float *)cos_out->data, (float *)sin_out->data,
                                                              max_seq_len, half, config->theta, config->head_dim);
    blt_cuda_launch_check("blt_rope_precompute");
}

__global__ void blt_rope_apply_kernel(const float *x, const float *cos, const float *sin, float *out, size_t seq_len,
                                      size_t num_heads, size_t head_dim, size_t half) {
    const size_t rows = seq_len * num_heads;
    for (size_t r = (size_t)blockIdx.x * blockDim.x + threadIdx.x; r < rows; r += (size_t)gridDim.x * blockDim.x) {
...
extern "C" void blt_rope_apply_cuda(const blt_tensor *x, const blt_tensor *cos, const blt_tensor *sin,
    ...
    blt_rope_apply_kernel<<<blt_cuda_grid(seq_len * num_heads), 256>>>((const float *)x->data, (const float *)cos->data,
                                                                       (const float *)sin->data, (float *)out->data,
...
__global__ void blt_rope_apply_backward_kernel(const float *grad_out, const float *cos, const float *sin,
                                               float *grad_in, size_t seq_len, size_t num_heads, size_t head_dim,
                                               size_t half) {
    const size_t rows = seq_len * num_heads;
    for (size_t r = (size_t)blockIdx.x * blockDim.x + threadIdx.x; r < rows; r += (size_t)gridDim.x * blockDim.x) {
        const size_t t = r / num_heads;
...
extern "C" void blt_rope_apply_backward_cuda(const blt_tensor *grad_out, const blt_tensor *cos, const blt_tensor *sin,
                                             blt_tensor *grad_in) {
    ...
    blt_rope_apply_backward_kernel<<<blt_cuda_grid(seq_len * num_heads), 256>>>(
        (const float *)grad_out->data, (const float *)cos->data, (const float *)sin->data, (float *)grad_in->data,
        seq_len, num_heads, head_dim, half);
...
__global__ void blt_rope_apply_packed_kernel(float *qkv_data, size_t qkv_stride, size_t head_offset, size_t seq_len,
...
extern "C" void blt_rope_apply_packed_cuda(float *qkv_data, size_t qkv_stride, size_t head_offset, size_t seq_len,
                                           size_t head_dim, const float *cos, const float *sin) {
    const size_t half = head_dim / 2;
    const size_t total = seq_len * half;
    const unsigned block = 256;
    ...
    blt_rope_apply_packed_kernel<<<blocks, block>>>(qkv_data, qkv_stride, head_offset, seq_len, head_dim, cos, sin);
    blt_cuda_launch_check("blt_rope_apply_packed");
}

__global__ void blt_rope_apply_packed_backward_kernel(float *qkv_data, size_t qkv_stride, size_t head_offset,
                                                      size_t seq_len, size_t head_dim, const float *cos,
                                                      const float *sin) {
    const size_t half = head_dim / 2;
    const size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    const size_t total = seq_len * half;
...
extern "C" void blt_rope_apply_packed_backward_cuda(float *qkv_data, size_t qkv_stride, size_t head_offset,
                                                    size_t seq_len, size_t head_dim, const float *cos,
                                                    const float *sin) {
    const size_t half = head_dim / 2;
    const size_t total = seq_len * half;
    ...
    blt_rope_apply_packed_backward_kernel<<<blocks, block>>>(qkv_data, qkv_stride, head_offset, seq_len, head_dim, cos,
                                                             sin);
...
__global__ void blt_rmsnorm_forward_kernel(const float *x, const float *w, float *out, size_t seq_len,
                                           size_t embed_dim) {
    for (size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x; i < seq_len; i += (size_t)gridDim.x * blockDim.x) {
...
extern "C" void blt_rmsnorm_forward_cuda(const blt_tensor *x, const blt_tensor *weight, blt_tensor *out) {
    ...
    blt_rmsnorm_forward_kernel<<<blt_cuda_grid(seq_len), 256>>>((const float *)x->data, (const float *)weight->data,
                                                                (float *)out->data, seq_len, embed_dim);
...
__global__ void blt_rmsnorm_backward_fused_kernel(const float *grad_out, const float *x, const float *w, float *grad_x,
                                                  float *grad_weight, size_t seq_len, size_t embed_dim) {
...
extern "C" void blt_rmsnorm_backward_cuda(const blt_tensor *grad_out, const blt_tensor *x, const blt_tensor *weight,
    ...
    blt_rmsnorm_backward_fused_kernel<<<1, threads, shared_mem>>>((const float *)grad_out->data, (const float *)x->data,
                                                                  (const float *)weight->data, (float *)grad_x->data,
...
__global__ void blt_layernorm_forward_kernel(const float *x, const float *w, const float *b, float *out, size_t seq_len,
                                             size_t embed_dim, float eps) {
    for (size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x; i < seq_len; i += (size_t)gridDim.x * blockDim.x) {
        const float *row = x + i * embed_dim;
        float *out_row = out + i * embed_dim;
...
extern "C" void blt_layernorm_forward_cuda(const blt_tensor *x, const blt_tensor *weight, const blt_tensor *bias,
                                           blt_tensor *out, float eps) {
    const size_t seq_len = x->shape[0];
    const size_t embed_dim = x->shape[1];
    blt_layernorm_forward_kernel<<<blt_cuda_grid(seq_len), 256>>>((const float *)x->data, (const float *)weight->data,
                                                                  (const float *)bias->data, (float *)out->data,
                                                                  seq_len, embed_dim, eps);
