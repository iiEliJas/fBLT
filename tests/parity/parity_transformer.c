...
int run_attention_parity_test(void) {
    ...
    char p[512];
    int ok = 1;
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_input.bin"), arena, &input);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_qkv_w.bin"), arena, &qkv_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_proj_w.bin"), arena, &proj_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_expected_out.bin"), arena, &expected_out);
    if (!ok) {
...
int run_attention_backward_parity_test(void) {
    ...
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_qkv_w.bin"), arena, &qkv_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_proj_w.bin"), arena, &proj_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_grad_out.bin"), arena, &grad_out);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_expected_grad_input.bin"), arena,
                             &expected_grad_input);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_expected_grad_qkv_w.bin"), arena,
                             &expected_grad_qkv_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "attn_expected_grad_proj_w.bin"), arena,
                             &expected_grad_proj_w);
    if (!ok) {
        blt_arena_destroy(arena);
        return 0;
    }
    ...
    blt_multihead_attention_backward(&input, &qkv_w, &proj_w, &grad_out, &grad_input, &grad_weight_qkv,
                                     &grad_weight_proj, &config, arena);
...
int run_transformer_block_parity_test(void) {
    ...
    char p[512];
    int ok = 1;
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_input.bin"), arena, &input);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_norm1_weight.bin"), arena, &norm1_weight);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_norm1_bias.bin"), arena, &norm1_bias);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_attn_qkv_w.bin"), arena, &attn_qkv_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_attn_proj_w.bin"), arena, &attn_proj_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_norm2_weight.bin"), arena, &norm2_weight);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_norm2_bias.bin"), arena, &norm2_bias);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_ffn_up_w.bin"), arena, &ffn_up_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_ffn_down_w.bin"), arena, &ffn_down_w);
    ok &= load_binary_tensor(blt_test_data_path(p, sizeof(p), "transformer_expected_out.bin"), arena, &expected_out);
    if (!ok) {
        blt_arena_destroy(arena);
        return 0;
    }
