...
static void build_scenario(blt_arena* host,
                           const blt_local_decoder_config* cfg,
                           blt_tensor* h_out, blt_tensor* patch_out,
                           blt_patch_info* patches, size_t* np_out,
                           uint8_t* clean_bytes, blt_block_batch* batch) {
    const size_t N = 16;
...
int run_cuda_parity_training_step(void) {
    ...
    {   // verify upload fidelity
        ...
        for (size_t i = 0; i < np; i++) {
            ...
            for (size_t e = 0; e < cpu_p[i]->numel; e++)
                worst0 = fmaxf(worst0, fabsf(c[e] - ref_check()[e]));
        }
    ...
    for (size_t step = 1; step <= 100; step++) {
        ...
        blt_local_decoder_forward_diffusion(cpu_d, &h, &patch_in, patches, num_patches,
            clean_bytes, &batch, BLT_D0_LEARNED, &cpu_logits, &cpu_loss, host);
        ...
        blt_local_decoder_forward_diffusion(dev_d, &d_h, &d_patch, patches, num_patches,
            clean_bytes, &batch, BLT_D0_LEARNED, &dev_logits, &dev_loss, dev);
        ...
        blt_local_decoder_backward_diffusion(cpu_d, &h, &patch_in, patches, num_patches,
            clean_bytes, &batch, BLT_D0_LEARNED, &g_bh, &g_pi, cpu_g, host);
        ...
        blt_local_decoder_backward_diffusion(dev_d, &d_h, &d_patch, patches, num_patches,
            clean_bytes, &batch, BLT_D0_LEARNED, &d_gbh, &d_gpi, dev_g, dev);
        ...
        // the milestone weight comparison).
        {
            ...
            float worst_g = 0.0f; size_t worst_gi = 0;
            ...
            for (size_t i = 0; i < ng; i++) {
                ...
                for (size_t e = 0; e < cpu_gr[i]->numel; e++) {
                    ...
                    if (d_ > worst_g) { worst_g = d_; worst_gi = i; }
                    if (strict) {
            ...
            fprintf(stderr, "  [grad parity] step %zu%s: max %.3g (grad %zu)\n",
                    step, strict ? " strict" : "", (double)worst_g, worst_gi);
        ...
        for (size_t m = mi; m < STEPS_MILESTONES; m++) {
            ...
            for (size_t i = 0; i < np; i++) {
                ...
                for (size_t e = 0; e < cpu_p[i]->numel; e++) {
                    ...
                    if (diff > worst_param) { worst_param = diff; worst_i = i; }
                }
            }
            fprintf(stderr, "  [training-step parity] step %zu: max diff %.3g (param %zu of %zu, numel %zu)\n",
                    step, (double)worst_param, worst_i, np, cpu_p[worst_i]->numel);
