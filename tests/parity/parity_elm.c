...
static void accumulate_grad(blt_entropy_lm_grad *acc, const blt_entropy_lm_grad *sample, const blt_entropy_lm *model) {
...
static float grad_global_norm(const blt_entropy_lm_grad *g, const blt_entropy_lm *model) {
    ...
    for (size_t l = 0; l < model->config.num_layers; ++l) {
        ...
        ss += sum_sq(&lg->norm1_weight) + sum_sq(&lg->attn_qkv_w) + sum_sq(&lg->attn_proj_w) +
              sum_sq(&lg->norm2_weight) + sum_sq(&lg->ffn_up_w) + sum_sq(&lg->ffn_gate_w) + sum_sq(&lg->ffn_down_w);
    }
    return sqrtf(ss);
}
...
static int run_overfit_batch(const uint8_t *pattern, size_t seq_len, float init_seed, float lr,
                             float near_zero_threshold, const char *label) {
    ...
    fprintf(stderr, "   [overfit %s] first_avg=%.4f last_avg=%.4f final=%.4f upticks=%d/%d -> %s\n", label, first_avg,
            last_avg, losses[OVERFIT_NUM_STEPS - 1], upticks, OVERFIT_NUM_STEPS - 1, ok ? "PASS" : "FAIL");
