...
static void fixed_patches(size_t seq_len, size_t patch_len,
                          blt_patch_info* out, size_t* num_out) {
    size_t start = 0;
    *num_out = 0;
    while (start < seq_len) {
...
static void setup_world(blt_arena** model_arena, blt_arena** scratch,
                        blt_model** model, blt_entropy_lm** lm,
...
static void clip_all(blt_model* m, blt_model_grad* g, float max_norm) {
    ...
    for (size_t part = 0; part < 3; part++) {
        const size_t n_layers =
            part == 1 ? m->global->stack.num_layers
                      : (part == 0 ? m->encoder->config.num_layers
                                   : m->decoder->config.num_layers);
        for (size_t i = 0; i < n_layers; i++) {
            if (part == 1) {
                blt_transformer_layer_grad* l =
                    &g->global_grad->stack_grad->layer_grads[i];
                blt_tensor* ts[] = {&l->norm1_weight, &l->attn_qkv_w,
                    &l->attn_proj_w, &l->norm2_weight, &l->ffn_up_w,
                    &l->ffn_gate_w, &l->ffn_down_w};
                for (size_t j = 0; j < 7; j++) blt_scale(ts[j], scale);
            } else {
                blt_local_layer_grad* l = part == 0
                    ? &g->encoder_grad->layer_grads[i]
                    : &g->decoder_grad->layer_grads[i];
                blt_tensor* ts[] = {&l->norm1_weight, &l->attn_qkv_w,
                    &l->attn_proj_w, &l->norm2_weight, &l->ffn_up_w,
                    &l->ffn_gate_w, &l->ffn_down_w,
                    &l->cross_norm_weight, &l->cross_weight_q,
                    &l->cross_weight_k, &l->cross_weight_v,
                    &l->cross_weight_proj};
...
static void apply_sgd_step_all(blt_model* model, blt_model_grad* grad, float lr) {
    ...
    for (size_t i = 0; i < enc->config.num_layers; i++) {
        ...
        blt_tensor* gs[] = {&g->norm1_weight, &g->attn_qkv_w, &g->attn_proj_w,
            &g->norm2_weight, &g->ffn_up_w, &g->ffn_gate_w, &g->ffn_down_w,
            &g->cross_norm_weight, &g->cross_weight_q, &g->cross_weight_k,
            &g->cross_weight_v, &g->cross_weight_proj};
...
static void train_snippets(blt_model* model, blt_model_grad* grad,
                           blt_arena* scratch, const char** snippets,
...
int run_blockgen_select_kernels(void) {
    // Confidence: threshold fires on all cells above alpha.
    {
        ...
        const uint32_t sel =
            blt_unmask_select_confidence(scores, masked, 4, 0.7f);
    ...
    // Confidence: nothing above alpha -> fallback picks the single best.
    {
        ...
        const uint32_t sel =
            blt_unmask_select_confidence(scores, masked, 3, 0.9f);
    ...
    // Confidence: unmasked cells never selected even with high scores.
    {
        ...
        const uint32_t sel =
            blt_unmask_select_confidence(scores, masked, 4, 0.5f);
...
int run_blockgen_draft_behavior(void) {
    ...
    uint8_t block_a[4];
    const size_t nfe_a = blt_draft_block(model, &enc, patches, num_patches,
                                         prompt, 12, &gc, block_a, scratch);
    TEST_ASSERT(nfe_a == 4);

    // Same call again: deterministic repeat.
    uint8_t block_b[4];
    const size_t nfe_b = blt_draft_block(model, &enc, patches, num_patches,
                                         prompt, 12, &gc, block_b, scratch);
    ...
    uint8_t block_c[4];
    const size_t nfe_c = blt_draft_block(model, &enc, patches, num_patches,
                                         prompt, 12, &gc, block_c, scratch);
    TEST_ASSERT(nfe_c == 1);
    ...
    uint8_t block_d[4];
    const size_t nfe_d = blt_draft_block(model, &enc, patches, num_patches,
                                         prompt, 12, &gc, block_d, scratch);
    TEST_ASSERT(nfe_d == 1);
...
int run_blockgen_generation_gates(void) {
    ...
    blt_generate_greedy_blockdiff(model, lm, &pcfg, prompt, prompt_len,
                                  new_bytes, out_a, &gc, NULL, scratch);
    TEST_ASSERT(memcmp(out_a, prompt, prompt_len) == 0);

    blt_infer_stats st;
    blt_generate_greedy_blockdiff(model, lm, &pcfg, prompt, prompt_len,
                                  new_bytes, out_b, &gc, &st, scratch);
