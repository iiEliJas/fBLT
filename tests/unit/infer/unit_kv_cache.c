...
static void build_scenario(blt_arena* arena, const dec_dims* dims,
                           size_t S, size_t patch_len,
                           blt_local_decoder** dec_out,
                           blt_tensor* h_out, blt_tensor* patch_in_out,
...
static int run_incremental(blt_arena* arena, blt_kv_cache* cache,
                           const blt_tensor* patch_in,
                           const blt_patch_info* patches, size_t num_patches,
                           const blt_tensor* h, size_t start,
                           const size_t* chunks, size_t num_chunks,
                           size_t V, blt_tensor* out) {
...
static int require_rows_equal(const blt_tensor* a, const blt_tensor* b,
...
static int run_pattern_case(const dec_dims* dims) {
    ...
    blt_local_decoder_forward_ext(dec, &h, &patch_in, patches, num_patches,
        NULL, NULL, 0, NULL, &ref, NULL, arena);
    ...
    // Pattern A: one big chunk
    {
        ...
        size_t chunks[1] = {S};
        TEST_ASSERT(run_incremental(arena, cache, &patch_in, patches, num_patches,
            &h, 0, chunks, 1, V, &got));
...
int run_kv_cache_rollback_commit(void) {
    ...
    blt_local_decoder_forward_ext(dec, &h2, &pin2, p2, np1, NULL, NULL, 0,
        NULL, &ref2, NULL, arena);
