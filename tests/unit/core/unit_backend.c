...
int run_backend_core_tests(void) {
    ...
    blt_add(&a, &b, &out);
    expected_add_data[0] = 3.0f;
    expected_add_data[1] = 4.0f;
    expected_add_data[2] = 5.0f;
    expected_add_data[3] = 6.0f;
    TEST_ASSERT_CLOSE(&out, &expected_add, 1e-6f);

    blt_mul(&a, &b, &out);
    expected_mul_data[0] = 2.0f;
    expected_mul_data[1] = 4.0f;
    expected_mul_data[2] = 6.0f;
    expected_mul_data[3] = 8.0f;
