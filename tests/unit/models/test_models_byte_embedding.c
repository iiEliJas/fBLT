...
int run_byte_embedding_model_tests(void) {
    ...
    /* Set distinct, known rows for byte values 0, 1, and 65 ('A') so we can
       verify the forward lookup pulls the correct row. */
    ...
    /* Row 0 of output should equal weight row for byte 65 */
    for (size_t d = 0; d < embed_dim; d++) {
        TEST_ASSERT(out_data[0 * embed_dim + d] == 10.0f + (float)d);
    }
    /* Row 1 of output should equal weight row for byte 0 */
    for (size_t d = 0; d < embed_dim; d++) {
        TEST_ASSERT(out_data[1 * embed_dim + d] == 0.0f);
    }
    /* Row 2 of output should equal weight row for byte 1 */
    for (size_t d = 0; d < embed_dim; d++) {
        TEST_ASSERT(out_data[2 * embed_dim + d] == 1.0f + (float)d);
    }

    /* Backward: scatter-add grad_out into grad_weight at the rows touched by
       bytes_in. Use two positions that hit the same byte value (1) to check
       accumulation. */
    bytes_data[0] = 1;
    bytes_data[1] = 1;
    bytes_data[2] = 2;
    ...
    /* blt_tensor_create zero-initializes storage, satisfying the "caller
       must zero before accumulating" requirement. */

    blt_byte_embedding_backward(&emb, &bytes_in, &grad_out, &grad_weight);

    float* grad_weight_data = (float*)grad_weight.data;
    /* byte 1 was hit twice (positions 0 and 1), so its grad row should be 2.0 */
    for (size_t d = 0; d < embed_dim; d++) {
        TEST_ASSERT(grad_weight_data[1 * embed_dim + d] == 2.0f);
    }
    /* byte 2 was hit once (position 2), so its grad row should be 1.0 */
    for (size_t d = 0; d < embed_dim; d++) {
        TEST_ASSERT(grad_weight_data[2 * embed_dim + d] == 1.0f);
    }
    /* an untouched byte row (e.g. byte 3) should remain zero */
    for (size_t d = 0; d < embed_dim; d++) {
