...
static int map_byte_boundaries_to_patch_boundaries(const size_t *byte_doc_boundaries, size_t num_docs,
...
int run_global_transformer_doc_boundary(void) {
    ...
    TEST_ASSERT(map_byte_boundaries_to_patch_boundaries(byte_doc_boundaries, num_boundaries, patches, num_patches,
                                                        patch_doc_boundaries));
    ...
    size_t bad_out[2] = {0};
    TEST_ASSERT(
        map_byte_boundaries_to_patch_boundaries(bad_boundaries, num_boundaries, patches, num_patches, bad_out) == 0);
...
static void sgd_update_global_transformer(blt_global_transformer *model, const blt_global_transformer_grad *grad,
                                          float lr) {
...
int run_global_transformer_overfit(void) {
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        blt_global_transformer_backward(model, &patch_in, doc_boundaries, num_docs, &grad_patch_out, &grad_patch_in,
                                        grad, arena);
