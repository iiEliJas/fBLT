...
static void gc_setup(blt_arena* arena, const tiny_dims* dims, gc_scenario* sc) {
    ...
    blt_block_batch_build(&sc->batch, arena, sc->bytes, sc->N,
        sc->patches, sc->num_patches, /*B=*/3, /*seed=*/42);
}

static float gc_loss(blt_arena* arena, blt_local_decoder* dec, const gc_scenario* sc,
                     blt_d0_mode mode) {
    ...
    blt_local_decoder_forward_diffusion(dec, &sc->h, &sc->pin, sc->patches, sc->num_patches,
        sc->bytes, &sc->batch, mode, &logits, &loss, arena);
...
int run_block_diffusion_gradcheck(void) {
    ...
    for (size_t i = 0; i < dec->config.num_layers; i++) {
        ...
        zero_tensor(&g->norm1_weight); zero_tensor(&g->attn_qkv_w);
        zero_tensor(&g->attn_proj_w);  zero_tensor(&g->norm2_weight);
        zero_tensor(&g->ffn_up_w);     zero_tensor(&g->ffn_gate_w);
        zero_tensor(&g->ffn_down_w);   zero_tensor(&g->cross_norm_weight);
        zero_tensor(&g->cross_weight_q); zero_tensor(&g->cross_weight_k);
        zero_tensor(&g->cross_weight_v); zero_tensor(&g->cross_weight_proj);
    }
    ...
    blt_local_decoder_backward_diffusion(dec, &sc.h, &sc.pin, sc.patches, sc.num_patches,
        sc.bytes, &sc.batch, mode, &grad_h, &grad_p, grad, scratch);
    ...
    struct probe { blt_tensor* t; size_t idx; float analytic; const char* name; };
    struct probe probes[] = {
    ...
    const blt_tensor* grad_map[] = {
        &grad->lm_head_grad, &grad->layer_grads[0].attn_qkv_w,
        &grad->layer_grads[0].attn_proj_w, &grad->layer_grads[0].ffn_down_w,
        &grad->layer_grads[0].ffn_gate_w, &grad->layer_grads[0].norm1_weight,
        &grad->layer_grads[0].norm2_weight, &grad->layer_grads[0].cross_weight_q,
        &grad->layer_grads[0].cross_norm_weight, &grad->d0_embed_grad,
    };
    ...
    for (size_t i = 0; i < sizeof(probes) / sizeof(probes[0]); i++) {
        ...
        printf("    %-14s analytic=%+.6f numeric=%+.6f rel=%.2e\n",
            probes[i].name, (double)analytic, (double)numeric, (double)rel);
    ...
    // input gradients: h[2][1] and patch_in[1][2]
    {
        ...
        printf("    %-14s analytic=%+.6f numeric=%+.6f rel=%.2e\n",
            "d/dh[2,1]", (double)analytic, (double)numeric, (double)rel);
        if (rel > 2e-2f) failures++;
        ...
        printf("    %-14s analytic=%+.6f numeric=%+.6f rel=%.2e\n",
            "d/dpin[1,2]", (double)analytic, (double)numeric, (double)rel);
        if (rel > 2e-2f) failures++;
    }
...
static void clip_all(blt_model* m, blt_model_grad* g, float max_norm) {
    ...
    for (size_t i = 0; i < m->encoder->config.num_layers; i++) {
        ...
        blt_tensor* ts[] = {&l->norm1_weight, &l->attn_qkv_w, &l->attn_proj_w,
    ...
    for (size_t i = 0; i < m->decoder->config.num_layers; i++) {
        ...
        blt_tensor* ts[] = {&l->cross_norm_weight, &l->cross_weight_q,
            &l->cross_weight_k, &l->cross_weight_v, &l->cross_weight_proj,
...
static void sgd_all(blt_model* m, blt_model_grad* g, float lr) {
    ...
    for (size_t i = 0; i < enc->config.num_layers; i++) {
        ...
        blt_tensor* ws[] = {&w->norm1_weight, &w->attn_qkv_w, &w->attn_proj_w,
            &w->norm2_weight, &w->ffn_up_w, &w->ffn_gate_w, &w->ffn_down_w,
            &w->cross_norm_weight, &w->cross_weight_q, &w->cross_weight_k,
            &w->cross_weight_v, &w->cross_weight_proj};
        blt_tensor* gs[] = {&gg->norm1_weight, &gg->attn_qkv_w, &gg->attn_proj_w,
            &gg->norm2_weight, &gg->ffn_up_w, &gg->ffn_gate_w, &gg->ffn_down_w,
            &gg->cross_norm_weight, &gg->cross_weight_q, &gg->cross_weight_k,
            &gg->cross_weight_v, &gg->cross_weight_proj};
    ...
    for (size_t i = 0; i < m->decoder->config.num_layers; i++) {
        ...
        blt_tensor* ws[] = {&w->cross_norm_weight, &w->cross_weight_q,
            &w->cross_weight_k, &w->cross_weight_v, &w->cross_weight_proj,
            &w->norm1_weight, &w->attn_qkv_w, &w->attn_proj_w,
            &w->norm2_weight, &w->ffn_up_w, &w->ffn_gate_w, &w->ffn_down_w};
        blt_tensor* gs[] = {&gg->cross_norm_weight, &gg->cross_weight_q,
            &gg->cross_weight_k, &gg->cross_weight_v, &gg->cross_weight_proj,
            &gg->norm1_weight, &gg->attn_qkv_w, &gg->attn_proj_w,
            &gg->norm2_weight, &gg->ffn_up_w, &gg->ffn_gate_w, &gg->ffn_down_w};
        for (size_t j = 0; j < 12; j++) blt_sgd_step(ws[j], gs[j], lr);
    }
...
int run_block_diffusion_overfit_gate(void) {
    ...
    cfg.encoder_config.num_layers = 1; cfg.encoder_config.hidden_dim = HID;
    cfg.encoder_config.num_heads = 2; cfg.encoder_config.cross_attn_heads = 2;
    cfg.encoder_config.local_window = 0; cfg.encoder_config.cross_attn_all_layers = true;
    cfg.encoder_config.pool_type = BLT_POOL_MEAN; cfg.encoder_config.rope_theta = 10000.0f;
    ...
    cfg.global_config.embed_dim = E; cfg.global_config.num_layers = 1;
    cfg.global_config.hidden_dim = HID; cfg.global_config.num_heads = 2;
    cfg.global_config.rope_theta = 10000.0f; cfg.global_config.max_seq_len = MS;
    cfg.decoder_config.embed_dim = E; cfg.decoder_config.patch_dim = 0;
    cfg.decoder_config.num_layers = 1; cfg.decoder_config.hidden_dim = HID;
    cfg.decoder_config.num_heads = 2; cfg.decoder_config.cross_attn_heads = 2;
    cfg.decoder_config.local_window = 0; cfg.decoder_config.cross_attn_all_layers = true;
    cfg.decoder_config.rope_theta = 10000.0f; cfg.decoder_config.max_seq_len = MS;
    ...
    for (size_t i = 0; i < model->encoder->config.num_layers; i++) {
        ...
        fill_constant(&l->norm1_weight, 1.0f);
        fill_small_uniform(&l->attn_qkv_w, 0.1f); fill_small_uniform(&l->attn_proj_w, 0.1f);
        fill_constant(&l->norm2_weight, 1.0f);
        fill_small_uniform(&l->ffn_up_w, 0.1f); fill_small_uniform(&l->ffn_gate_w, 0.1f);
        fill_small_uniform(&l->ffn_down_w, 0.1f);
        fill_constant(&l->cross_norm_weight, 1.0f);
        fill_small_uniform(&l->cross_weight_q, 0.1f); fill_small_uniform(&l->cross_weight_k, 0.1f);
        fill_small_uniform(&l->cross_weight_v, 0.1f); fill_small_uniform(&l->cross_weight_proj, 0.1f);
    }
    for (size_t i = 0; i < model->global->stack.num_layers; i++) fill_t_layer(&model->global->stack.layer_storage[i], 0.1f);
    ...
    fill_small_uniform(&model->decoder->lm_head_weight, 0.1f);
    {
        ...
        for (size_t i = 0; i < model->decoder->d0_embed_weight.numel; i++)
            tbl[i] = (((i * 13) % 17) - 8.0f) * 0.05f;
    }
    ...
    for (size_t step = 0; step < STEPS; step++) {
        ...
        for (size_t sn = 0; sn < num_snippets; sn++) {
            ...
            blt_local_decoder_forward_diffusion(model->decoder, &h, &O, patches, M,
                text, &batch, BLT_D0_LEARNED, &logits, &loss, scratch);
            ...
            blt_local_decoder_backward_diffusion(model->decoder, &h, &O, patches, M,
                text, &batch, BLT_D0_LEARNED, &grad_h, &grad_O, grad->decoder_grad, scratch);
