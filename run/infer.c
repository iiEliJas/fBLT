...
static void usage(void) {
    fprintf(stderr,
        "usage: bin/infer --checkpoint FILE --embed E --hidden H --enc-layers N "
...
static const char *method_name(int method) {
    switch (method) {
    case M_GREEDY:   return "greedy";
    case M_SELFSPEC:  return "selfspec";
    case M_BLOCKDIFF: return "blockdiff";
    case M_BLOCKDV:   return "blockdv";
...
int main(int argc, char **argv) {
    ...
    BLT_REQUIRE(a.embed > 0,
                "infer: --embed is required");
    BLT_REQUIRE(a.hidden > 0,
                "infer: --hidden is required");
    BLT_REQUIRE(a.enc_layers > 0,
                "infer: --enc-layers is required");
    BLT_REQUIRE(a.glob_layers > 0,
                "infer: --glob-layers is required");
    BLT_REQUIRE(a.dec_layers > 0,
                "infer: --dec-layers is required");
    BLT_REQUIRE(a.use_cuda >= 0,
                "infer: --backend cpu|cuda is required");
    ...
    fprintf(stderr, "[infer] patcher: %s\n",
            a.fixed_patches ? "fixed-stride-4"
                            : "entropy (thr, max)");
    ...
    blt_make_patcher_cfg(&pcfg, a.fixed_patches, a.patch_threshold_global, a.patch_threshold_monotonic, a.max_patch_length);
    ...
    switch (a.method) {
    case M_GREEDY:
        blt_generate_greedy(model, lm, &pcfg, prompt_bytes, prompt_len,
                            a.new_bytes, output + prompt_len, scratch);
    ...
    case M_SELFSPEC: {
        ...
        blt_generate_greedy_selfspec(model, lm, &pcfg, prompt_bytes, prompt_len,
                                     a.new_bytes, output + prompt_len, &sc, &st,
                                     scratch);
        break;
    }
    case M_BLOCKDIFF: {
        ...
        blt_generate_greedy_blockdiff(model, lm, &pcfg, prompt_bytes, prompt_len,
                                      a.new_bytes, output + prompt_len, &gc, &st,
                                      scratch);
        break;
    }
    case M_BLOCKDV: {
        ...
        blt_generate_greedy_blockdiff_verify(model, lm, &pcfg, prompt_bytes,
                                             prompt_len, a.new_bytes,
                                             output + prompt_len, &gc, &st,
                                             scratch);
        break;
    }
    }
