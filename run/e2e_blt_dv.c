...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--ckpt") && i + 1 < argc) ckpt_path = argv[++i];
            ...
            new_bytes = strtoull(argv[++i], NULL, 10);
    ...
    blt_generate_greedy_blockdiff(model, lm, &pcfg, prompt, prompt_len,
                                  new_bytes, draft_only, &gc, NULL, scratch);
