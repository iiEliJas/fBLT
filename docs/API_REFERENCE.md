...
- `blt_block_batch_build_t(out, arena, bytes, N, patches, num_patches, B, rng_seed, t_in)`
  - Same as `blt_block_batch_build` with a caller-supplied timestep: `t_in > 0` fixes both the masking probability and the 1/t loss weight (curriculum schedules); `t_in <= 0` samples `t ~ U(0,1)` from the seed exactly like the plain build. Masking draws still consume the seeded RNG stream.
...
- Inference benchmark: `make bench-infer` builds `bin/infer_bench` (bench/infer_bench.c). Loads the plain + BLT-D checkpoints, generates from deep held-out prompts, and appends per-config JSONL rows (phase "6_infer") to bench/results.jsonl via the shared harness: decoder/encoder NFEs per byte, speculative acceptance, agreement with the same model's greedy output (verified rows gate on `--dv-gate`, default 0.90; 0 disables the fatal check), wall-clock stats. Checkpoint shape flags: `--embed E --hidden H --enc-layers N --glob-layers N --dec-layers N --cross-attn all|last` must match the trained weights. Configs: greedy baseline, BLT-S k in {4,8,16} (both models), BLT-D / BLT-DV with B in {4,8,16}, confidence alpha=0.7, one-step (alpha=0), EB gamma variants, and the diffusion add-on methods — `blockdv_hetv` (draft BLT-D, verify with the plain model, agreement judged against plain greedy), `blockdv_bal` (boundary-aligned commitment), `blockdv_adapt` (adaptive B within [4,16] targeting 0.5 rolling acceptance over 8 rounds) and their combination. Plots: `python3 tools/bench_plots.py` renders PNGs into graphs/.
- BLT-D / BLT-DV generation controllers (`include/blt/infer/block_generation.h`, `src/infer/block_generation.c`)
  ...
  - `blt_draft_block`: Algorithm 1 inner loop over frozen latents; returns decoder NFEs; never resets the scratch arena (caller owns lifetime via round markers).
   ...
   - `blt_block_gen_config {block_size, d0_mode, opts{strategy, threshold α|γ, use_top_p, top_p, seed}, verifier_model, boundary_aligned, B_min, B_max, accept_target, adapt_window}`.
     - `verifier_model` (NULL default): when set, verification runs through this model instead of the drafting model (heterogeneous DV: draft with BLT-D, verify with plain). NULL keeps classic self-verification.
     - `boundary_aligned` (0 default): commits are cut back to natural candidate patch boundaries via `blt_verify_draft_aligned`, mirroring training-time block construction (blocks start at patch starts).
     - `B_max > 0 && adapt_window > 0` enables adaptive block sizing: per-round acceptance feeds a rolling average over `adapt_window` rounds and `blt_block_adapt_b` moves the block size one step within `[B_min, B_max]` toward `accept_target` (±0.10 deadband).
   - `blt_block_adapt_b(cur_b, rolling_acceptance, b_min, b_max, target)`: pure adaptive-B rule; clamps into bounds first, then grows/shrinks one byte when acceptance leaves the ±0.10 deadband around `target`. `b_max == 0` or `b_max < b_min` disables (returns clamped `cur_b`).
- Weight checkpoints (`include/blt/models/checkpoint.h`, `src/models/checkpoint.c`)
  ...
  - Save/load is backend-aware: device-resident tensors are staged through host buffers (`blt_tensor_download` / `blt_tensor_upload`); CPU tensors take the direct fread/fwrite path unchanged. The same staging applies to `blt_entropy_lm_save` / `blt_entropy_lm_load`.
...
- Trainer entry point: `make train-blt-d` builds `bin/train_blt_d` for full-pipeline BLT-D / plain-BLT runs:
  ...
  - `--backend cpu|cuda`: run model + training passes on the chosen backend (CUDA build required for `cuda`; all host/device crossings — byte ids in, loss/logits/entropies out — are staged; the entropy-LM patcher stays host-side by design and `--train-entropy-lm` always runs on CPU). Grad clipping uses backend-dispatched `blt_vec_dot(x, x)`; checkpoint I/O stages through host memory.
  - `--enc-layers N --glob-layers N --dec-layers N`: per-submodule layer counts (0 = use `--layers`). Decoupling supports decoder-depth sweeps without touching encoder/global.
  - `--cross-attn all|last`: cross-attention placement in the local encoder + decoder (default `all`; `last` fires only after the final local layer).
  - `--t-warmup-hi F --t-hi-start F2`: high-t curriculum — over the first `F * steps` steps the sampled diffusion timestep is held above a floor decaying linearly from `F2` to `--t-min`, so early training emphasizes heavily-masked blocks before annealing into the full U(0,1) distribution (`--t-warmup-hi 0` disables).
  ...
  - `--mask-late-step N --mask-late-scale F`: raise the cap from `--mask-scale` toward `F` linearly between step N and the end (gentle late-ramp retry axis)
  - `--entropy-patches --entropy-lm FILE`: segment training/eval windows with the entropy LM + patcher instead of fixed stride 4
...
- `blt_verify_draft(model, entropy_model, patcher_config, x, l, r, target_len, stats, arena)`
  ...
  - Behavior: Fast-BLT Algorithm 2 — re-segments the whole candidate via the entropy patcher, runs one full encoder+global pass and a logits-only decode, accepts drafted bytes up to the first mismatch (replacing it with the verified prediction) or commits all of them plus one free byte on a full match. A patch boundary is forced at the commit point `l` before encoding so the prefix-side segmentation matches greedy's own re-segmentation. Updates `stats` (`nfe_encoder_global`, `nfe_decoder`, `bytes_accepted`) when provided. This is the shared verification step reused by BLT-DV.
- `blt_verify_draft_aligned(model, entropy_model, patcher_config, x, l, r, target_len, stats, arena)`
  - Boundary-aligned variant of `blt_verify_draft`: same verify pass but the candidate is segmented naturally (no forced split at `l`) and commitment is cut back to the largest natural patch end in `(l, verified_len]`; draft bytes past that boundary are discarded and re-drafted next round. On a mismatch with no boundary inside the verified range, exactly the corrected byte is committed (progress rule). Full match commits `l+r` (the tiling end is always a boundary) and still appends the free byte when budget allows. Returns the new committed length in `[l+1, l+r+1]`.
  - Rationale: training blocks start at patch starts (`blt_block_batch_build`); aligned commits reproduce that construction at inference and keep every commit point on a boundary a streaming patcher reproduces in longer contexts.
- `blt_aligned_commit_select(patches, num_patches, lo, hi)`
  - Pure helper behind the aligned commit rule: returns the largest patch end `e` with `lo < e <= hi`, or 0 when none exists. Unit-testable without a model.
