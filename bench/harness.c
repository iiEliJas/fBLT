...
int bench_write_json(const char* path, const bench_result* r) {
    ...
    if (fprintf(f, "{\"timestamp\":%lld,\"phase\":\"%s\",\"name\":\"%s\",\"tag\":\"%s\",",
                (long long)now, r->phase, r->name, r->tag) < 0) {
        fclose(f);
        return -1;
    }

    if (fprintf(f, "\"latency\":{\"n\":%zu,\"mean\":%.9e,\"stddev\":%.9e,"
                   "\"min\":%.9e,\"max\":%.9e,\"p50\":%.9e,\"p90\":%.9e,\"p99\":%.9e}",
                r->latency.n, r->latency.mean, r->latency.stddev,
                r->latency.min, r->latency.max,
                r->latency.p50, r->latency.p90, r->latency.p99) < 0) {
        fclose(f);
        return -1;
    }
