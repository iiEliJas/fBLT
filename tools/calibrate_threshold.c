...
int calibrate_threshold(const char* entropy_file, float target_patch_size, float* calibrated_threshold, calibration_stats* out_stats){
    ...
    *calibrated_threshold = theta_best;
    memcpy(out_stats, &stats, sizeof(calibration_stats));
...
int main(int argc, char** argv){
    ...
    float calibrated_threshold = 0.0f;
    calibration_stats stats;

    int result = calibrate_threshold(argv[1], target_patch_size, &calibrated_threshold, &stats);
