...
double blt_flops_cross_attention(size_t l, size_t h_k, size_t n_heads, size_t patch_size, double r) {
...
double blt_flops_transformer_block(size_t l, size_t h, size_t m, size_t n_heads, size_t h_k, size_t d_ff,
                                   size_t vocab_size) {
...
double blt_flops_per_byte(const blt_flops_config *cfg) {
    ...
    double FL_global = blt_flops_transformer_block(cfg->l_G, cfg->h_G, global_ctx, cfg->global_heads,
                                                   cfg->global_head_dim, cfg->d_ff_G,
    ...
    // Eq. 14: local encoder
    double FL_encoder =
        blt_flops_transformer_block(cfg->l_E, cfg->h_E, cfg->w_E, cfg->enc_heads, cfg->enc_head_dim, cfg->d_ff_E,
                                    0 // vocab_size = 0, encoder has no de-embedding
        );

    // Eq. 15: local decoder
    double FL_decoder =
        blt_flops_transformer_block(cfg->l_D, cfg->h_D, cfg->w_D, cfg->dec_heads, cfg->dec_head_dim, cfg->d_ff_D,
                                    cfg->vocab_size // 256
        );

    // Eq. 16: encoder cross-attention, scaled by k/n_p
    double FL_cross_E = blt_flops_cross_attention(cfg->l_E, cfg->h_E, cfg->enc_heads, cfg->n_p, n_p / k) * (k / n_p);

    // Eq. 17: decoder cross-attention
    double FL_cross_D = blt_flops_cross_attention(cfg->l_D, cfg->h_D, cfg->dec_heads, cfg->k, k / n_p);
