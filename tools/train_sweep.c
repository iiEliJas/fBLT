...
static void fill_constant(blt_tensor *t, float v) { blt_fill_constant(t->backend, (float *)t->data, t->numel, v); }
...
static void warn_unknown_keys(const blt_json_value *obj, const char *const *known, size_t n_known, const char *ctx) {
    if (!obj || obj->type != BLT_JSON_OBJECT) return;
    for (size_t i = 0; i < obj->num_children; i++) {
        bool found = false;
        for (size_t k = 0; k < n_known; k++) {
            if (strcmp(obj->keys[i], known[k]) == 0) {
                found = true;
                break;
            }
        }
...
static void parse_placement(const char *s, sweep_config *c) {
    if (strcmp(s, "none") == 0) {
        c->enc_xattn = BLT_XATTN_NONE;
        c->dec_xattn = BLT_XATTN_NONE;
    } else if (strcmp(s, "encoder_all") == 0) {
        c->enc_xattn = BLT_XATTN_ALL;
        c->dec_xattn = BLT_XATTN_NONE;
    } else if (strcmp(s, "encoder_last") == 0) {
        c->enc_xattn = BLT_XATTN_LAST;
        c->dec_xattn = BLT_XATTN_NONE;
    } else if (strcmp(s, "decoder_all") == 0) {
        c->enc_xattn = BLT_XATTN_NONE;
        c->dec_xattn = BLT_XATTN_ALL;
    } else if (strcmp(s, "decoder_first") == 0) {
        c->enc_xattn = BLT_XATTN_NONE;
        c->dec_xattn = BLT_XATTN_FIRST;
...
static void load_config(const char *path, sweep_config *c) {
    ...
    static const char *top_keys[] = {"name", "tag", "phase", "model", "cross_attention", "patcher", "train"};
    ...
    static const char *pa_keys[] = {"rule", "threshold_global", "threshold_monotonic", "max_patch_length"};
    warn_unknown_keys(pa, pa_keys, 4, "patcher");
    parse_patcher_rule(blt_json_get_string(pa, "rule", "global"), c);
    c->patcher.threshold_global = (float)blt_json_get_float(pa, "threshold_global", 1.0);
    c->patcher.threshold_monotonic = (float)blt_json_get_float(pa, "threshold_monotonic", 0.5);
    c->patcher.max_patch_length = (size_t)blt_json_get_int(pa, "max_patch_length", 64);

    const blt_json_value *tr = blt_json_get(root, "train");
    static const char *tr_keys[] = {"steps",        "lr",           "seq_len",         "seed",          "eval_every",
                                    "train_bin",    "heldout_bin",  "heldout_c_bin",   "heldout_h_bin", "ckpt_every",
                                    "max_wall_min", "eval_windows", "ent_warmup_steps"};
    ...
    case PATCH_RULE_ENTROPY_MONOTONIC:
        c->patcher.rule = BLT_PATCH_RULE_MONOTONIC;
        break;
    case PATCH_RULE_ENTROPY_BOTH:
        c->patcher.rule = BLT_PATCH_RULE_BOTH;
        break;
    default:
        break;
    }
...
static void build_model_config(const sweep_config *c, blt_model_config *mc) {
    ...
    mc->encoder_config.pool_type = c->pooling_init_mean ? BLT_POOL_MEAN : BLT_POOL_MAX;
...
static bool uses_entropy_patcher(const sweep_config *c) {
    return c->patch_rule == PATCH_RULE_ENTROPY_GLOBAL || c->patch_rule == PATCH_RULE_ENTROPY_MONOTONIC ||
           c->patch_rule == PATCH_RULE_ENTROPY_BOTH;
}
...
static void windows_from_stream(size_t stream_len, size_t seq_len, long max_windows, window_list *wl) {
...
static void windows_from_manifest(const byte_buf *manifest_bytes, const char *split, size_t seq_len, long max_windows,
                                  window_list *wl) {
...
static size_t build_fixed_patches(size_t seq_len, size_t stride, blt_patch_info *out) {
    size_t n = 0, start = 0;
    while (start < seq_len) {
...
static bool is_ws_break(uint8_t c) { return c == ' ' || c == '\n' || c == '\t' || c == '\r'; }

static size_t build_whitespace_patches(const uint8_t *bytes, size_t seq_len, size_t max_len, blt_patch_info *out) {
    size_t n = 0, start = 0;
    while (start < seq_len) {
        size_t len = 0;
        while (start + len < seq_len && len < max_len && !(len > 0 && is_ws_break(bytes[start + len - 1]))) {
            len++;
        }
        if (len == 0) len = 1;
        out[n].start_idx = start;
...
static size_t make_patches(const train_ctx *tc, const uint8_t *window_bytes, size_t seq_len, blt_tensor *bytes_in,
    ...
    if (!uses_entropy_patcher(tc->cfg)) {
        ...
        return build_whitespace_patches(window_bytes, seq_len, tc->cfg->patcher.max_patch_length, patches);
    }
    ...
    size_t n = blt_segment_patches(&ent_vals_host, window_bytes, patches, seq_len, &tc->cfg->patcher);
    free(ent_host);
    return n;
}
...
static float model_grad_sq_norm(blt_model_grad *g, const blt_model *m) {
    ...
    for (size_t l = 0; l < m->encoder->config.num_layers; l++) {
        ...
        total += tensor_sq_norm(&s->norm1_weight) + tensor_sq_norm(&s->attn_qkv_w) + tensor_sq_norm(&s->attn_proj_w) +
                 tensor_sq_norm(&s->norm2_weight) + tensor_sq_norm(&s->ffn_up_w) + tensor_sq_norm(&s->ffn_gate_w) +
    ...
    for (size_t l = 0; l < m->decoder->config.num_layers; l++) {
        ...
        total += tensor_sq_norm(&s->cross_norm_weight) + tensor_sq_norm(&s->cross_weight_q) +
                 ...
                 tensor_sq_norm(&s->attn_qkv_w) + tensor_sq_norm(&s->attn_proj_w) + tensor_sq_norm(&s->norm2_weight) +
                 tensor_sq_norm(&s->ffn_up_w) + tensor_sq_norm(&s->ffn_gate_w) + tensor_sq_norm(&s->ffn_down_w);
    }
...
static void scale_model_grads(blt_model_grad *g, const blt_model *m, float s) {
    ...
    for (size_t l = 0; l < m->encoder->config.num_layers; l++) {
        ...
        blt_tensor *all[12] = {&t->norm1_weight,   &t->attn_qkv_w,     &t->attn_proj_w,    &t->norm2_weight,
                               &t->ffn_up_w,       &t->ffn_gate_w,     &t->ffn_down_w,     &t->cross_norm_weight,
                               &t->cross_weight_q, &t->cross_weight_k, &t->cross_weight_v, &t->cross_weight_proj};
        for (int i = 0; i < 12; i++) blt_scale(all[i], s);
    }
    for (size_t l = 0; l < m->global->stack.num_layers; l++) {
        blt_transformer_layer_grad *t = &g->global_grad->stack_grad->layer_grads[l];
        blt_tensor *all[7] = {&t->norm1_weight, &t->attn_qkv_w, &t->attn_proj_w, &t->norm2_weight,
                              &t->ffn_up_w,     &t->ffn_gate_w, &t->ffn_down_w};
        for (int i = 0; i < 7; i++) blt_scale(all[i], s);
    }
    for (size_t l = 0; l < m->decoder->config.num_layers; l++) {
        blt_local_layer_grad *t = &g->decoder_grad->layer_grads[l];
        blt_tensor *all[12] = {&t->cross_norm_weight, &t->cross_weight_q, &t->cross_weight_k, &t->cross_weight_v,
                               &t->cross_weight_proj, &t->norm1_weight,   &t->attn_qkv_w,     &t->attn_proj_w,
                               &t->norm2_weight,      &t->ffn_up_w,       &t->ffn_gate_w,     &t->ffn_down_w};
        for (int i = 0; i < 12; i++) blt_scale(all[i], s);
    }
...
static void zero_scatter_grads(blt_model_grad *g, const blt_local_encoder *enc, const blt_model_config *cfg) {
    ...
    for (size_t l = 0; l < enc->config.num_layers; l++) {
        blt_local_layer_grad *s = &g->encoder_grad->layer_grads[l];
        zero_tensor(&s->norm1_weight);
        zero_tensor(&s->attn_qkv_w);
        zero_tensor(&s->attn_proj_w);
        zero_tensor(&s->norm2_weight);
        zero_tensor(&s->ffn_up_w);
        zero_tensor(&s->ffn_gate_w);
        zero_tensor(&s->ffn_down_w);
        zero_tensor(&s->cross_norm_weight);
        zero_tensor(&s->cross_weight_q);
        zero_tensor(&s->cross_weight_k);
        zero_tensor(&s->cross_weight_v);
        zero_tensor(&s->cross_weight_proj);
    }
...
static void sgd_layer_transformer(blt_transformer_layer_storage *w, blt_transformer_layer_grad *g, float lr) {
...
static void sgd_apply_model(blt_model *m, blt_model_grad *g, float lr) {
    ...
    for (size_t i = 0; i < m->encoder->ngram_weights.num_tables; i++) {
        blt_sgd_step(&m->encoder->ngram_weights.tables[i], &g->encoder_grad->ngram_grads.tables[i], lr);
    }
    for (size_t l = 0; l < m->encoder->config.num_layers; l++) {
    ...
    for (size_t l = 0; l < m->global->stack.num_layers; l++) {
        sgd_layer_transformer(&m->global->stack.layer_storage[l], &g->global_grad->stack_grad->layer_grads[l], lr);
    }
    for (size_t l = 0; l < m->decoder->config.num_layers; l++) {
...
static void sgd_apply_entropy_lm(blt_entropy_lm *m, blt_entropy_lm_grad *g, float lr) {
    ...
    for (size_t l = 0; l < m->stack.num_layers; l++) {
        sgd_layer_transformer(&m->stack.layer_storage[l], &g->stack_grad->layer_grads[l], lr);
    }
...
static void eval_stream(const train_ctx *tc, const byte_buf *buf, const window_list *wl, blt_arena *arena,
                        eval_out *out) {
    ...
    out->bpb = (positions > 0) ? loss_sum / (log(2.0) * (double)positions) : 0.0;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--config") == 0 && i + 1 < argc) {
            ...
            if (strcmp(b, "cpu") == 0) backend = BLT_BACKEND_CPU;
                ...
                fprintf(stderr, "Invalid backend: %s (cpu|cuda)\n", b);
                return 2;
            }
        } else {
            usage(argv[0]);
    ...
    printf("[sweep] tag=%s phase=%s steps=%ld seq_len=%zu lr=%g seed=%llu\n", cfg.tag, cfg.phase, total_steps,
           cfg.seq_len, cfg.lr, (unsigned long long)cfg.seed);
    ...
    windows_from_manifest(&manifest_buf, "heldout", cfg.seq_len, cfg.eval_windows, &held_wl);
    ...
    double *step_samples =
        (double *)malloc(sizeof(double) * (total_steps - start_step > 0 ? (size_t)(total_steps - start_step) : 1));
    ...
    if (ent_lm && cfg.ent_warmup_steps > 0 && start_step < cfg.ent_warmup_steps) {
        ...
        for (long ws = start_step; ws < cfg.ent_warmup_steps; ws++) {
            ...
            const uint8_t *wb = train_buf.data + train_wl.items[ws % (long)train_wl.num].offset;
            size_t bshape[1] = {L};
    ...
    for (long step = main_loop_start; step < total_steps; step++) {
        ...
        const uint8_t *wbytes = train_buf.data + train_wl.items[step % (long)train_wl.num].offset;
        size_t bytes_shape[1] = {L};
        ...
        if (ent_lm) {
            ...
            blt_tensor ent_logits = blt_tensor_create(scratch, ent_logits_shape, 2, BLT_DTYPE_FP32);
            blt_tensor ent_loss = blt_tensor_create(scratch, scalar_shape, 1, BLT_DTYPE_FP32);
        ...
        if ((step + 1) % cfg.eval_every == 0) {
            ...
            printf("[sweep] step %ld/%ld train_loss=%.4f bpb=%.4f avg_patch=%.2f "
                   ...
                   step + 1, total_steps, loss_val, eo.bpb, eo.avg_patch_len, dt * 1e3, now_sec() - t_start);
            fflush(stdout);
        }
    }
    ...
    if (eo.bpb > 0.0) {
        bench_result_add_metric(&r, "flops_per_byte", blt_flops_per_byte(&fc));
    }
    bench_result_add_metric(&r, "throughput_bytes_per_sec", bytes_trained / wall);
